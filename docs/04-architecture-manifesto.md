...
packages/
  ...
  backend/
    apps/                   # scope:backend, type:app — run, never imported
      api/                  # @repel/backend-api     — HTTP facade; entrypoint src/main.ts
      cli/                  # @repel/backend-cli     — CLI facade;  entrypoint src/index.ts
      worker/               # @repel/backend-worker  — processing-pipeline runner host
    libs/                   # scope:backend, type:lib — importable
      ...
      processing-pipeline/  # @repel/backend-processing-pipeline  async orchestrator
      transport/            # @repel/backend-transport            the queue
...
**Apps are not importable.** An app's `package.json` declares no `exports` and no `main`; nothing can resolve `@repel/backend-api` as a module. The app's role (api / cli / worker) is selected by which entrypoint a process runs — there is no `MODE` switch and no single combined `main.ts` (see ADR-003). Each app is a thin composer over libs.
