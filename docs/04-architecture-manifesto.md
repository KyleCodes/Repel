...
packages/
  ...
  backend/
    ...
    libs/                   # scope:backend, type:lib — importable
      accounts/             # @repel/backend-accounts  a product-capability domain lib
        persistence/        #   service.ts mutations/ views/ transform.ts lib/ error.ts
      sync/                 # @repel/backend-sync   sync domain — owns persistence AND adapter integration
        persistence/        #   service.ts mutations/ views/ contract.ts transform.ts lib/
        async/              #   the executor + per-event handler (worker-facing)
      adapters/             # @repel/backend-adapters  provider integrations (gmail: ingress, egress, normalize)
      queue/                # @repel/backend-queue  client + Postgres transport + consumer runtime
...
**A package's public surface is its `exports` map** (ADR-015): the source files other packages may import; everything else is private. A domain lib like `@repel/backend-accounts` exports only `./service` and `./error` — its `persistence/` internals are unreachable from outside, which is how Rule 1b is enforced across packages (see §5).

Adding a product capability adds a domain lib under `<platform>/libs/` (`@repel/backend-<domain>`); adding an external provider adds a directory under `@repel/backend-adapters`. Every new lib carries `type:lib` + its `scope:` tag. A new deployable is a `type:app` package under a stack directory, exposing only `./start`, plus a compose service. Nothing is added at the top level, and no new tag dimension is introduced, without amending this document.
...
**Mutation.** A synchronous write: one async function taking a `Tx` and a typed input, running one SQL statement (a writeable CTE when more than one table is involved), returning the Kysely-inferred shape. Mutations live at `libs/<domain>/persistence/mutations/<verb-noun>.ts`. They do not open transactions and are not decorated; the service does. ("Flow" is reserved for multi-step processes like an OAuth flow, not DB writes.)

**View.** A synchronous read — same shape as a mutation but read-only. Views live at `libs/<domain>/persistence/views/<verb-noun>.ts`.

**Handler.** The work a queue consumer runs for one envelope — same internal shape as a mutation (typed input/result, tenant transaction), but invoked by `consume(topic, handler)`, not a facade. It does its work and calls `enqueue(topic, payload)` directly for downstream work; there is no router fanning out a return value. The handler body is app code (in the consumer app, or a domain's `async/`); the queue lib knows nothing of what it does.

**Service.** The public surface of a domain — exactly one `service.ts` per lib. It imports mutations/views, decorates each operation with `runInTx`/`runInOrgTx`, applies business rules, and owns the transform between its public interfaces and the kysely query shapes (see Queries). `runInOrgTx` adds `orgId: string` to the public input; the inner mutation/view never declares it — `SET LOCAL` RLS does the tenant filtering. Services throw errors extending the domain's `error.ts` base; facades catch and map by `instanceof`.

**Queries.** Kysely expressions inside the mutation or view that uses them. Each declares a private builder (`buildX(trx, input) => …`), derives its result type by inference (`InferResult<ReturnType<typeof buildX>>[number]`), and exports it alongside the runner. There is no per-domain query bundle — Kysely's `DB` schema in `@repel/backend-db` is the source of truth, and return types reach the app through inference. Use raw `sql` only when Kysely cannot express the statement.

**Below the service, query inputs are kysely-derived**, not hand-written: a write-value member is `Insertable<Table>` (or `Updateable<Table>`) narrowed with `Omit<…>`; a where-key member is `Pick<Selectable<Table>, …>`; each uses a table-namespaced envelope (one member per table) so the call site says which entity a column belongs to. `orgId` is the exception — never enveloped; it is the `runInOrgTx` tenant scope, stripped by the decorator before the query (RLS filters, the query does not).

**At the service boundary,** a method may either reuse the kysely-derived input/result as pure passthrough, or declare its own application interface (in `contract.ts`) and map to/from the kysely shapes in `transform.ts` — one named input map and one output map per diverging method (REP-56). The transform is service-owned, exactly as a facade owns its wire↔service reshape (Rule 4). The query layer never sees the application types. Cross-domain reads still go through `views/` per Rule 1b.
...
**Rule 5. Domain-lib internal structure: `persistence/` and (when there is async work) `async/`.** A capability is a `libs/<domain>` package. Its `persistence/` directory holds the synchronous data layer; `async/` holds worker-facing handlers.

- **`persistence/service.ts`** — exactly one. Decorates mutations/views with `runInTx`/`runInOrgTx`, owns business rules and the transform seam, and is the only import target for facades and other domains' services.
- **`persistence/mutations/<verb-noun>.ts`** — one file per write use case: `async (trx, input) => …` owning its builder and inferred result type. Split into a `<verb-noun>/` step-file dir only past ~150 lines.
- **`persistence/views/<verb-noun>.ts`** — one file per read use case; like a mutation but read-only.
- **`persistence/contract.ts`** + **`persistence/transform.ts`** — public service interfaces and their maps to/from the kysely shapes, for methods whose contract diverges (see Queries). Omit when every method is passthrough.
- **`persistence/lib/`** — reusable helpers consumed within the package (e.g. typed event constructors wrapping the service); distinct from the package's own public exports.
- **`persistence/error.ts`** — abstract domain base extending `AppError`, plus concrete subclasses thrown by service methods.
- **`async/`** — the executor and per-event/per-envelope handlers (`libs/queue` consumers); same internal shape as a mutation.

Each mutation/view is the smallest unit of DB work — one query, one inferred type, one runner. `service.ts` is the only file that opens transactions. The shape rules out both the per-entity quartet and the per-domain query bundle: every read or write is owned by the file that names the use case. Avoid `service/` as a directory name (it reads as a running process); split a large `service.ts` by operation cluster within `persistence/`.

**Rule 6. The queue owns no business logic; consumers own only orchestration.** `libs/queue` holds the envelope type, the client (`enqueue`/`consume`), the Postgres transport, and the consumer runtime (claim, deliver, retry, dead-letter, drain) — nothing about what any handler does. A handler unpacks its typed payload, opens the tenant transaction, calls a service, and (if needed) enqueues the next envelope; domain knowledge lives in services and adapters, never in the queue or the wiring.

The queue is replaceable infrastructure: swapping the Postgres transport for Inngest/Temporal/RabbitMQ changes `libs/queue`, not handlers. Handlers are unit-testable without a queue — feed an envelope, assert on DB state and enqueued envelopes.

**Rule 7. Schemas are colocated with routes and commands.** Validation schemas live next to the route or command they validate, not in a central `schemas/` directory — a corollary of file-based routing (§6): a route directory holds everything needed to serve it (handler, schema, reshape helpers), so reading the directory is enough.

**Rule 8. Module boundaries are tag-enforced; every package declares its own dependencies.** Every package carries Nx tags — `type:app|lib|cli` and `scope:backend|frontend|shared`. `@nx/enforce-module-boundaries` fails `nx lint` on any import violating the table below; `@nx/dependency-checks` fails it on any import of a package the importer's own `package.json` does not declare. Both run pre-commit.
...
| `type:cli`       | `type:lib`, `type:app`           | the toolbox may launch apps (`./start`)         |
| `scope:shared`   | `scope:shared`                   | shared is isomorphic                            |
| `scope:backend`  | `scope:backend`, `scope:shared`  | backend ↛ frontend                              |
| `scope:frontend` | `scope:frontend`, `scope:shared` | frontend ↛ backend                              |

The `type:cli → type:app` edge is the one licensed app-import: the cli imports an app's `./start` to launch it. `type:app ↛ type:app` holds, so no app imports another. The dependency-declaration clause keeps the graph honest — a package may not import `zod`, `pg`, or a sibling `@repel/*` it resolves only via hoisting; it must be in that package's `package.json` (test-only deps in `devDependencies`) or `nx lint` fails.

`nx sync` generates each package's `tsconfig` `references` from the dependency graph (`nx sync:check` fails on drift); only the root solution `tsconfig.json` is hand-maintained, so a new package must be added there.

> **Note (REP-56):** an earlier Rule 6 forbade `area:feature ↔ area:adapter` imports, enforced by an `area:` tag dimension on the single `@repel/backend-features` and `@repel/backend-adapters` packages. When `features` dissolved into per-domain libs (`libs/sync`, `libs/accounts`; see §3, §4 Feature), a domain lib came to own its persistence _and_ its adapter integration end-to-end — `libs/sync` legally imports both `@repel/backend-adapters` and `@repel/backend-db`. The wall and its tag dimension were removed. Channel-agnosticism is now preserved where it actually lives: the queue envelope between adapter ingress and a domain handler (§8), not a package boundary.
...
The architectural commitment in the product specification — that async processing operates on normalized messages and is channel-agnostic — is realized by the queue envelope between adapter ingress and the domain handlers that consume it (adapters speak only the envelope contract) plus the egress routing pattern (only the egress consumer knows which providers exist). Adding a provider is a new directory under `adapters/`, a new egress routing entry, and zero or more new topics — never a change inside a domain lib.
