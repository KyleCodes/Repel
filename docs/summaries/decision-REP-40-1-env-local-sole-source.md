...
- Per-worktree contract: `repel db clone` writes `.env.local` (mode 0o600) for each worktree, scoped to that worktree's branch DB. Reading from `.env.local` directly means the CLI always targets the _correct_ DB for the current worktree, regardless of any stale `DATABASE_URL` in the user's shell environment.
