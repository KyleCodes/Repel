...
- **Current mechanism:** each role is its own app package under `packages/backend/apps/` — `@repel/backend-api`, `@repel/backend-sync-worker`, and others as stacks are added. The role is chosen by **which app runs**, not by an env var. There is no `MODE` switch and no generic `worker` host (the former placeholder `@repel/backend-worker` was deleted; consumers are named per-stack apps such as `sync-worker`). An app owns its app logic (HTTP server, request parsing, consumer instantiation, handler bodies) and delegates to the shared `libs/`. Each app's `src/main.ts` is both its direct entrypoint (`bun run .../main.ts`, guarded by `import.meta.main`) and its single public export, `./start` (its launch function), consumed only by the cli (see ADR-015); the app is otherwise un-importable.
- **The launcher.** The cli (`@repel/backend-cli`, tagged `type:cli`, not `type:app`) is the launcher: `cli services run <name>` resolves config, renders env, and calls an app's `start()`; `cli services run --all` calls every app's `start()` in one process for a single-debugger end-to-end run. The cli path is not mandatory: because each app's `main.ts` is also a direct entrypoint, a process (dev or Compose) can run an app directly via `bun run <app>/src/main.ts`. This keeps optionality — if launching production through the cli proves not worth it, `cli services run` can remain a development-workflow convenience while Compose runs apps directly. There is no compile step. Multiple compiled artifacts (one per app, via `bun build --compile`) replace per-app `bun run` when staging/prod deploy is built out — deferred, not yet present.
