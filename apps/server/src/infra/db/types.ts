...
// `Tx` is the strict service-layer type: every feature operation runs
// inside a transaction, and `make<Feature>Queries(trx: Tx)` factories
// close over it. `DbExecutor` (the union) is retained for the admin-ops
// layer that legitimately operates against the maintenance DB outside
// any tx context.
export type Db = Kysely<DB>;
export type Tx = Transaction<DB>;
export type DbExecutor = Db | Tx;

// Row type aliases used by feature query bundles. Services map row → domain
// inline with each query (no separate mappers layer; manifesto Rule 5).
